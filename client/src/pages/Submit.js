...
export default function Submit() {
  ...
  const handleMatchSave = (e) => {
    ...
    const winner = context.find((e) => e.abbr === data.winner);
    const loser = context.find((e) => e.abbr === data.loser);
    console.log(winner._id, loser._id);
  ...
  return (
    <div>
      ...
      <p>
        {data.winner}
        {data.winnerRace} vs {data.loser}
        {data.loserRace} on {data.map}
      </p>
      <form>
      ...
      {error.email !== "" ? <h3>{error.email}</h3> : <div></div>}
      {error.password !== "" ? <h3>{error.password}</h3> : <div></div>}
    </div>
  );
}
